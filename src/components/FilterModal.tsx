...
export const FilterModal = (props: FilterModalProps) => {
  ...
  const SelectChip = ({ label, isActive, onClick }: { label: string; isActive: boolean; onClick: () => void }) => (
    <button
      onClick={onClick}
      className={`px-3 py-2 rounded-lg text-xs font-bold transition-all border ${isActive ? 'bg-indigo-50 border-indigo-200 text-indigo-600 shadow-sm' : 'bg-white border-slate-200 text-slate-600 hover:border-indigo-200'}`}
    >
      {label}
    </button>
  );
    ...
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center bg-slate-900/20 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full md:w-[600px] md:rounded-3xl rounded-t-3xl shadow-2xl flex flex-col max-h-[90vh] md:max-h-[85vh] animate-in slide-in-from-bottom-10 duration-300">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-100">
          ...
          <button onClick={props.onClose} className="w-8 h-8 rounded-full bg-slate-100 text-slate-400 hover:bg-slate-200 flex items-center justify-center">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 scrollbar-hide">
          ...
          {/* Priority 1: Date & Range (Top) */}
          <div className="flex flex-col gap-6 bg-slate-50 p-4 rounded-2xl border border-slate-100">
            {/* Date */}
            <div className="flex flex-col gap-2">
               <SectionTitle icon="fa-calendar-days" label="Date Added (Primary)" />
               <div className="flex items-center gap-2">
                  <input type="date" value={props.dateFrom} onChange={(e) => props.setDateFrom(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-100" />
                  <span className="text-slate-400">~</span>
                  <input type="date" value={props.dateTo} onChange={(e) => props.setDateTo(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-100" />
               </div>
            </div>
            <div className="h-px bg-slate-200 w-full" />
            {/* Range */}
            <div className="flex flex-col gap-2">
               <SectionTitle icon="fa-list-ol" label="Word Index (#) (Primary)" />
               <div className="flex items-center gap-2">
                  <input type="number" placeholder="1" min="1" value={props.indexFrom} onChange={(e) => props.setIndexFrom(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-center text-slate-700 outline-none focus:ring-2 focus:ring-indigo-100" />
                  <span className="text-slate-400">~</span>
                  <input type="number" placeholder="End" min="1" value={props.indexTo} onChange={(e) => props.setIndexTo(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-center text-slate-700 outline-none focus:ring-2 focus:ring-indigo-100" />
               </div>
            </div>
          </div>
