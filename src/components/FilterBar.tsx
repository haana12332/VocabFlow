        ...
        <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-3 overflow-x-auto scrollbar-hide pb-1">
                <button
                    ...
                    className={`
                      flex items-center gap-2 px-5 h-10 rounded-xl font-bold text-xs transition-all flex-shrink-0 shadow-sm
                ...
                {props.category !== 'All' && (
                    <ActiveFilterBadge label={props.category} onClear={() => props.setCategory('All')} />
                )}
                {props.status !== 'All' && (
                    <ActiveFilterBadge label={props.status} onClear={() => props.setStatus('All')} />
                )}
                {props.pos !== 'All' && (
                    <ActiveFilterBadge label={props.pos} onClear={() => props.setPos('All')} />
                )}
                {props.toeic !== 'All' && (
                    <ActiveFilterBadge label={`TOEIC > ${props.toeic}`} onClear={() => props.setToeic('All')} />
                )}
                {(props.dateFrom || props.dateTo) && (
                    <ActiveFilterBadge label="Date Range" onClear={() => { props.setDateFrom(''); props.setDateTo(''); }} />
                )}
                {(props.indexFrom || props.indexTo) && (
                    <ActiveFilterBadge label="Index Range" onClear={() => { props.setIndexFrom(''); props.setIndexTo(''); }} />
                )}
                
                {props.isAnyFilterActive && (
                    <button 
                        onClick={props.onResetAll}
                        className="text-[10px] font-bold text-slate-400 hover:text-red-500 underline ml-auto px-2 whitespace-nowrap"
                    >
                        Clear All
                    </button>
                )}
